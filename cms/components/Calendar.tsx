...
function EventChip({ event }: { event: CalEvent }) {
  const base = "text-xs px-1.5 py-0.5 rounded truncate max-w-full";
  if (event.type === "meeting") {
    return (
      <div className={clsx(base, "bg-blue-600/80 text-blue-100")} title={event.title}>
  ...
  return (
    <div className={clsx(base, STATUS_COLOR[event.status ?? "todo"])} title={event.title}>
      {event.title}
    </div>
  );
}
...
function MonthlyView({ year, month, events }: { year: number; month: number; events: CalEvent[] }) {
  const today = ymd(new Date());
  // Days in month, first day of month
  const firstDay = new Date(year, month, 1);
  const daysInMonth = new Date(year, month + 1, 0).getDate();
  const startOffset = (firstDay.getDay() + 6) % 7; // Mon=0 offset
  ...
  function eventsOnDay(day: number) {
    ...
    return events.filter(e => isInRange(d, e.startDate, e.endDate));
  }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-7 gap-px bg-zinc-800 rounded-xl overflow-hidden border border-zinc-800">
        {cells.map((day, i) => {
          if (!day) return <div key={i} className="bg-zinc-950 min-h-[80px] md:min-h-[100px]" />;
          const d = `${year}-${String(month + 1).padStart(2, "0")}-${String(day).padStart(2, "0")}`;
          const dayEvents = eventsOnDay(day);
          const isToday = d === today;
          return (
            <div key={i} className={clsx("bg-zinc-950 p-1 min-h-[80px] md:min-h-[100px]", isToday && "bg-zinc-900/80")}>
              <span className={clsx(
                "text-xs font-medium w-6 h-6 flex items-center justify-center rounded-full mb-1",
                isToday ? "bg-red-600 text-white" : "text-zinc-500",
              )}>{day}</span>
              <div className="space-y-0.5">
                {dayEvents.slice(0, 3).map(e => <EventChip key={e.id + d} event={e} />)}
                {dayEvents.length > 3 && (
...
function WeeklyView({ weekStart, events }: { weekStart: Date; events: CalEvent[] }) {
  const today = ymd(new Date());
          ...
          return (
            <div key={i} className={clsx("bg-zinc-950 p-1.5 min-h-[160px]", isToday && "bg-zinc-900/80")}>
              <div className="space-y-1">
                ...
                {dayEvents.map(e => <EventChip key={e.id + d} event={e} />)}
              </div>
            </div>
          );
        })}
...
export default function Calendar({ tasks, meetings }: Props) {
  ...
  // Convert tasks + meetings to CalEvents
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6 gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-zinc-100 w-52 text-center">{headerLabel}</h2>
      ...
      {view === "monthly"
        ? <MonthlyView year={year} month={month} events={events} />
